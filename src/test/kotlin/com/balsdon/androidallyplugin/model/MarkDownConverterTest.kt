...
class MarkDownConverterTest {
    ...
    fun no_elements_selected_generate_list_without_not_applicable_and_none_selected() {
        ...
        assertThat(lines.filter { line -> line == "## Settings" }.size)
            .isEqualTo(1)
        assertThat(lines.filter { line -> line == "- [ ] Orientation: Landscape and portrait are supported ([orientation][7])" }.size)
            .isEqualTo(1)
        ...
        assertThat(lines.filter { line -> line == "- [ ] Captions: Videos have non-automated captions ([caption preferences][10], [w3c captions][11], [3PlayMedia][12])" }.size)
            .isEqualTo(1)
        assertThat(lines.filter { line -> line == "- [ ] Touch Targets: Elements with which users interact have a minimum target size of 48 x 48dp (checked with accessibility scanner) ([touch targets][16])" }.size)
            .isEqualTo(1)
    ...
    fun one_element_selected_generate_list_without_not_applicable_and_one_selected() {
        ...
        assertThat(lines.filter { line -> line == "- [ ] Touch Targets: Elements with which users interact have a minimum target size of 48 x 48dp (checked with accessibility scanner) ([touch targets][16])" }.size)
            .isEqualTo(1)
        assertThat(lines.filter { line -> line == "- [x] Actions: Accessibility actions have been created for secondary actions on card list items ([actions][17])" }.size)
            .isEqualTo(1)
        assertThat(lines.filter { line -> line == "[0]: https://support.google.com/accessibility/android/answer/6376570?hl=en-GB" }.size)
            .isEqualTo(1)
    }
    ...
    fun no_elements_selected_generate_list_with_not_applicable_and_none_selected() {
        ...
        assertThat(lines.filter { line -> line == "- ~~[ ] Enough time: Time to take action setting is respected ([enough time][8])~~" }.size)
            .isEqualTo(1)
        assertThat(lines.filter { line -> line == "- ~~[ ] Pause, Stop, Hide: All video content can be paused, stopped and hidden. No video flashes more than 3 times per second ([pause stop hide][19])~~" }.size)
            .isEqualTo(1)
